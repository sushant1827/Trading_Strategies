...
def run_strategy_with_params(df, st_period, st_multiplier, zz_depth, zz_backstep, confirmation_mode='trend_following'):
    ...
    try:
        ...
        pending_buy_signal_idx = None
        pending_sell_signal_idx = None

        for i in range(1, len(df_full)-1):  # Adjusted to allow checking i+1
            ...
            # Check for entry if pending and at next candle (i+1) - DO THIS BEFORE SETTING NEW PENDING
            if pending_buy_signal_idx is not None and i == pending_buy_signal_idx + 1:
                signal_candle_green = df_full['Close'].iloc[pending_buy_signal_idx] > df_full['Open'].iloc[pending_buy_signal_idx]
                next_candle_green = df_full['Close'].iloc[i] > df_full['Open'].iloc[i]
                if signal_candle_green or next_candle_green:
                    if not in_buy_position and not in_sell_position and last_pivot_label in buy_confirm_patterns:
                        df_full.loc[current_idx, 'Buy_Entry'] = df_full['Open'].iloc[i]
                        in_buy_position = True
                pending_buy_signal_idx = None

            if pending_sell_signal_idx is not None and i == pending_sell_signal_idx + 1:
                signal_candle_red = df_full['Close'].iloc[pending_sell_signal_idx] < df_full['Open'].iloc[pending_sell_signal_idx]
                next_candle_red = df_full['Close'].iloc[i] < df_full['Open'].iloc[i]
                if signal_candle_red or next_candle_red:
                    if not in_sell_position and not in_buy_position and last_pivot_label in sell_confirm_patterns:
                        df_full.loc[current_idx, 'Sell_Entry'] = df_full['Open'].iloc[i]
                        in_sell_position = True
                pending_sell_signal_idx = None

            # Check for Supertrend signal at current i - AFTER CHECKING PENDING ENTRIES
            if df_full[st_dir_col_name].iloc[i] == 1 and df_full[st_dir_col_name].iloc[i-1] != 1:
                pending_buy_signal_idx = i
            elif df_full[st_dir_col_name].iloc[i] == -1 and df_full[st_dir_col_name].iloc[i-1] != -1:
                pending_sell_signal_idx = i
...
def main():
    ...
    st_periods = [20, 21, 22]
    st_multipliers = [0.8, 0.9, 1.0, 1.2]
    zz_depths = [25, 30, 35]
    ...
    # Save results
    results_df.to_csv('zigzag_supertrend_new_optimization_results.csv', index=False)
