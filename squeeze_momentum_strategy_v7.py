...
def example_usage():
    ...
    for tf in timeframes:
        ...
        # Initialize indicator with optimal parameters
        squeeze_indicator = SqueezeMomentumIndicator()
        params = squeeze_indicator.optimize_for_timeframe(tf)
        
        print(f"Parameters: {params['description']}")
        print(f"BB Length: {params['bb_length']}, BB Mult: {params['bb_mult']}")
        print(f"KC Length: {params['kc_length']}, KC Mult: {params['kc_mult']}")
        
        # Run backtest with risk management
        backtest_results = squeeze_indicator.backtest_strategy(
            df, 
    ...
    # Demonstrate the enhanced strategy with daily timeframe
    print("\n=== Enhanced Strategy Demo (Daily Timeframe) ===")
    
    # Initialize with daily parameters
    squeeze_indicator = SqueezeMomentumIndicator()
    squeeze_indicator.optimize_for_timeframe('1d')
    
    # Calculate indicators with enhanced logic
    result = squeeze_indicator.generate_signals(df)
    
    # Display sample results
    print("\nSample of enhanced indicators:")
    columns_to_show = ['close', 'squeeze_on', 'squeeze_release', 'momentum', 
                      'momentum_increasing', 'two_bar_rule', 'signal', 'position']
    print(result[columns_to_show].tail(15))
    
    # Detailed backtest
    detailed_backtest = squeeze_indicator.backtest_strategy(
        df, 
        initial_capital=10000,
        stop_loss_pct=2.5,
        take_profit_pct=5.0
    ...
    print(f"Total Return: {detailed_backtest['total_return']:.2f}%")
    print(f"Final Capital: ${detailed_backtest['final_capital']:.2f}")
    print(f"Number of Trades: {detailed_backtest['num_trades']}")
    print(f"Win Rate: {detailed_backtest['win_rate']:.1f}%")
    print(f"Average Win: {detailed_backtest['avg_win']:.2f}%")
    print(f"Average Loss: {detailed_backtest['avg_loss']:.2f}%")
    print(f"Max Drawdown: {detailed_backtest['max_drawdown']:.2f}%")
    
    # Show last few trades
    if detailed_backtest['trades']:
        print(f"\nLast 5 Trades:")
        for trade in detailed_backtest['trades'][-5:]:
            print(f"Entry: {trade['entry_price']:.2f}, Exit: {trade['exit_price']:.2f}, "
                  f"P&L: {trade['pnl_percent']:.2f}%, Reason: {trade['exit_reason']}")
    
    # Plot the indicator
    try:
        squeeze_indicator.plot_indicator(df.tail(100), 
                                       title="Enhanced Squeeze Momentum Strategy")
    except:
...
def create_trading_strategy_summary():
    """Create a comprehensive strategy summary"""
    
    summary = """
    === ENHANCED TTM SQUEEZE MOMENTUM STRATEGY ===
    
    KEY IMPROVEMENTS (Based on 2024 Research):
    
    1. TIMEFRAME OPTIMIZATION:
       - 15m: Fast parameters (BB/KC length=10) for scalping
       - 1h: Standard parameters for short-term trading  
       - 4h: Standard parameters for swing trading
       - 1d: Tighter Keltner (mult=1.2) for better squeeze detection
       - 1w: Shorter periods (length=15) for long-term trends
    
    2. ENHANCED ENTRY RULES:
       - Wait for squeeze release (first green dot after red dots)
       - Confirm positive momentum that's increasing
       - Filter false signals with momentum strength checks
    
    3. IMPROVED EXIT RULES (John Carter's "Two Bar Rule"):
       - Exit after two consecutive bars of momentum decrease
       - Exit on momentum crossing below zero
       - Optional stop-loss and take-profit levels
    
    4. RISK MANAGEMENT:
       - Configurable stop-loss (recommended: 2-3%)
       - Take-profit targets (recommended: 5-6%)
       - Maximum drawdown tracking
    
    5. BEST PRACTICES:
       - Most effective on 1-hour and 4-hour timeframes
       - Works best in trending markets
       - Combine with volume confirmation for better results
    ...
    USAGE:
    1. Load your OHLC data into a pandas DataFrame
    2. Choose appropriate timeframe parameters
    3. Run the strategy with risk management
...
if __name__ == "__main__":
    ...
    create_trading_strategy_summary()import pandas as pd
import numpy as np
