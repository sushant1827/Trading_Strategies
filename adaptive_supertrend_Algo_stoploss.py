...
class Backtester:
    def __init__(self, initial_capital=1000000, commission_rate=0.001, stop_loss_atr_multiplier=2.0):
        ...
        if signal == 'buy' and self.current_position <= 0:
            ...
            if position_size > 0:
                ...
                if position_size > 0:
                    ...
                    self.stop_loss_level = price - atr * self.stop_loss_atr_multiplier
...
def run_ml_supertrend_backtest(df):
    ...
    # Initialize backtester with stop-loss
    backtester = Backtester(initial_capital=100000, commission_rate=0.003, stop_loss_atr_multiplier=2.0)
    ...
    if len(analyzer.trades_df) > 0:
        analyzer.trades_df.to_csv('adaptive_supertrend_stoploss_trades.csv', index=False)
        print(f"\nTrades saved to adaptive_supertrend_stoploss_trades.csv")
