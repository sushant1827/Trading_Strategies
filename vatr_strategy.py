...
def compute_vatr(df, atr_len=14, atr_ma_len=50, lookback=20, m_b_base=1.0, s=1.0, m_ts_base=1.5):
    df = df.copy()
    ...
    df['indicator'] = df['break_high'] + df['m_b'] * df['atr']
...
def generate_signals(df):
    df = df.copy()
    # breakout: close crosses above breakout level -> long
    df['above'] = df['close'] > df['indicator']
    df['above_prev'] = df['above'].shift(1).fillna(False)
    df['cross_up'] = (~df['above_prev']) & (df['above'])
    df['long_signal'] = df['cross_up']
    # exit: price falls below trailing level or reverse breakout
    df['exit_signal'] = df['close'] < df['vatr_trail']
    return df

def backtest(df, start_equity=100000, risk_pct=0.01, m_init=2.0, warmup=30):
    ...
    for i in range(warmup, len(df)-1):
        ...
        if pos==0 and df.at[i,'long_signal']:
            ...
            entry_atr = df.at[i,'atr']; stop = entry_price - m_init * entry_atr
...
def load_or_synth(path=None):
    if path:
        df = pd.read_csv(path, parse_dates=True, index_col=0); df.columns=[c.lower() for c in df.columns]; return df
    rng = np.random.default_rng(2); n=1000; dt=pd.date_range(end=pd.Timestamp.today(), periods=n, freq='D')
    steps = rng.normal(0.0003,0.01,size=n); close=100+np.cumsum(steps); open_=np.roll(close,1); open_[0]=close[0]
    high=np.maximum(open_,close)+abs(rng.normal(0,0.3,n)); low=np.minimum(open_,close)-abs(rng.normal(0,0.3,n))
...
if __name__=='__main__':
    ...
    print("Trades:", len(trades)); print(trades.head() if not trades.empty else "No trades")
    ...
    plt.figure(figsize=(12,3)); eq['equity'].plot(); plt.show()
