...
def calculate_price_action_features(df):
    ...
    # Returns over past 1, 2, 3 days
    df['Return_1day'] = df['close'].pct_change(periods=1) * 100
    df['Return_2day'] = df['close'].pct_change(periods=2) * 100
    df['Return_3day'] = df['close'].pct_change(periods=3) * 100
...
def define_target_variable(df, price_increase_threshold=0.05, prediction_horizon_days=5):
    """
    Defines the target variable: 1 if price increases by threshold in next X days, else 0.
    """
    print(f"Defining target variable (price increase by {price_increase_threshold}% in next {prediction_horizon_days} days)...")
...
def run_trading_strategy(df, price_increase_threshold=0.05, prediction_horizon_days=5,
    ...
    Args:
        ...
        prediction_horizon_days (int): How many days into the future to predict.
    ...
    df = define_target_variable(df, price_increase_threshold, prediction_horizon_days)
    ...
    backtest_results = f"""
Backtest Results:
Accuracy: {accuracy_score(y_test, y_pred_backtest):.4f}
Precision: {precision_score(y_test, y_pred_backtest):.4f}
Recall: {recall_score(y_test, y_pred_backtest):.4f}
F1-Score: {f1_score(y_test, y_pred_backtest):.4f}

Confusion Matrix (Backtest):
{confusion_matrix(y_test, y_pred_backtest)}
"""
    print(backtest_results)
    with open('original_ensemble_results.txt', 'w') as f:
        f.write("Original Ensemble Daily Strategy Results\n")
        f.write("=" * 50 + "\n")
...
Forward Test Results:
Accuracy: {accuracy_score(y_forward_test, y_pred_forward_test):.4f}
Precision: {precision_score(y_forward_test, y_pred_forward_test):.4f}
Recall: {recall_score(y_forward_test, y_pred_forward_test):.4f}
F1-Score: {f1_score(y_forward_test, y_pred_forward_test):.4f}

Confusion Matrix (Forward Test):
{confusion_matrix(y_forward_test, y_pred_forward_test)}

Forward Test Strategy Simulation:
Number of predicted 'buy' signals (trades): {num_trades_forward}
Total simulated return from trades: {total_strategy_return_forward:.2f}%
""" + (f"Average return per trade: {total_strategy_return_forward / num_trades_forward:.2f}%" if num_trades_forward > 0 else "No trades were made in forward test.")

        print(forward_results)
        with open('original_ensemble_results.txt', 'a') as f:
            f.write(forward_results)
    else:
...
if __name__ == "__main__":
    # Define the path to the Nifty50 daily data
    nifty_data_path = r'D:\Sushant\Fyers_AlgoTrade\Fyers_Data\Nifty50_Index\NIFTY50_INDEX_D_Min.csv'
    ...
    run_trading_strategy(ohlc_data.copy(),
                         price_increase_threshold=0.05, # Target: 0.05% price increase
                         prediction_horizon_days=5,   # Predict 5 days ahead
