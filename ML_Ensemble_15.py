...
# Load the data
data = pd.read_csv('SomeStrategy15.csv')
...
daily_summary = data.groupby('Date_only').agg({'Open': 'first', 'Close': 'last'}).reset_index()

data['Pred'] = daily_summary['Open'] < daily_summary['Close']
# data['Pred'] = data['Open'] < data['Close'].
shift_count = len(daily_summary)
data['Pred'] = data['Pred'].astype(int).shift(shift_count)

data.drop(data.tail(shift_count).index,inplace=True)

# data['Date'] = pd.to_datetime(data['Date'])
data.set_index('Date', inplace=True)

data.to_csv('ML_Ensemble15.csv')
...
lag = 25  # Use the last 5 rows for prediction
