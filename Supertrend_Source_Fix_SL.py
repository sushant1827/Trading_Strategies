...
def run_optimization(
    ...
    for _ in range(num_iterations):
        ...
        # - Current HA_Close is above Supertrend 1.
        # - Supertrend 1 direction is upward (1).
        ...
        # Calculate StopLoss based on the second supertrend (st_2).
        # For an upward trend (direction 1), stop loss is trendingUp band.
        # For a downward trend (direction -1), stop loss is trendingDown band.
        df_copy["StopLoss"] = np.where(
            df_copy[st_2_dir_name] == 1,
            df_copy[st_2_trending_up_name],
            df_copy[st_2_trending_down_name],
        )
        df_copy["StopLoss"] = df_copy["StopLoss"].shift(1)
        ...
        # - Current HA_Close is below Supertrend 1.
        # - Supertrend 1 direction is downward (-1).
        ...
        if not df_copy.empty:
            ...
            if (
                performance_metrics["profit_factor"].iloc[0] > 1
                and performance_metrics["win_rate"].iloc[0] >= 50
...
if __name__ == "__main__":
    ...
    try:
        ...
        period_ranges = [(25, 25, 1), (20, 20, 1)] # Example: (start, end, step) for period 1 and period 2
        multiplier_ranges = [(1.0, 1.0, 1), (1.2, 1.4, 0.1)] # Example: (start, end, step) for multiplier 1 and multiplier 2
        ...
        optimization_results = run_optimization(
            ...
            brokerage_pct=0.0003,
            slippage_points=2,
            num_iterations=10, # Number of random combinations to test.
        )
        ...
        if not optimization_results.empty:
            ...
            # Calculate StopLoss for the best strategy using the second Supertrend.
            best_df_copy["StopLoss"] = np.where(
                best_df_copy[st_2_best_dir_name] == 1,
                best_df_copy[st_2_best_trending_up_name],
                best_df_copy[st_2_best_trending_down_name],
            )
            best_df_copy["StopLoss"] = best_df_copy["StopLoss"].shift(1)
