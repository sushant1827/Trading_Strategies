...
def calculate_ut_bot(df_input, open_col, high_col, low_col, close_col, key_val=1, atr_period=10):
    ...
    # Signals
    buy_signal_base = ((df['src'] > df['xATRTrailingStop']) & 
                       (df['ema1'] > df['xATRTrailingStop']) & 
                       (df['ema1'].shift() <= df['xATRTrailingStop'].shift()))
    
    sell_signal_base = ((df['src'] < df['xATRTrailingStop']) & 
                        (df['ema1'] < df['xATRTrailingStop']) & 
                        (df['ema1'].shift() >= df['xATRTrailingStop'].shift()))
...
def calculate_metrics(df_signals, initial_capital=100000, trade_size=1):
    df = df_signals.copy()
...
key_vals = [2.1]  # Key values to test
atr_periods = [20]  # ATR periods to test
# key_vals = [1.5]  # Key values to test
# atr_periods = [25]  # ATR periods to test
print("\nStarting strategy optimization...")

for source_name, cols in ohlc_sources.items():
    for key_val in key_vals:
        for atr_period in atr_periods:
            print(f"Testing config: Source={source_name}, KeyVal={key_val}, ATRPeriod={atr_period}")
            ...
            metrics_result = calculate_metrics(df_with_signals)
