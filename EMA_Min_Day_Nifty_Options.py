...
symbol = 'Nifty'
signal_filename = r'D:\AlgoTrade\Fyers_AlgoTrade\EMA_Day_17_EMA_Mins_17-15_Mins_Signals.csv'
...
for index, row in df.iterrows():
    ...
    current_date   = row['date_15min']
    current_time   = row['time_15min']
    current_spot   = row['Open_15min']
    # current_spot   = row['Open']

    strike_offset = 0
    if current_signal == 'Buy' : strike_offset = 0 #200
    if current_signal == 'Sell': strike_offset = 0 #-200
...
trade_df.to_csv('EMA_Min_Hour_Nifty_Options' + '.csv') 
