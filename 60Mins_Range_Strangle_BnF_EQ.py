...
vix_df = vix_df.iloc[-2919:-42]
# vix_df = vix_df.iloc[-308:-42]
...
options_path = 'D:/AlgoTrade/Fyers_AlgoTrade/Fyers_Data/New_Options_Data_BnF/2023-2024'
...
columns = [
    'Date', 'Day_Name', 'Start_Time', 'Spot_Strike', 'CE_Strike', 'PE_Strike', 'Lot_Size', 'CE_Sell_Price', 'PE_Sell_Price', 
...
for current_day in pd.date_range(start=start_date, end=end_date):
    ...
    time_delta1 = timedelta(hours=trade_time.hour-1, minutes=trade_time.minute, seconds=trade_time.second) # removed 1 hr to get spot time candle
    first_candle = current_day + time_delta1

    spot_day = spot_df[(spot_df['Date'] == first_candle)]

    if spot_day.empty:
        continue

    CE_spot_price = spot_day.iloc[0]['High']
    # CE_spot_price = spot_day.iloc[0]['Close'] if spot_day.iloc[0]['Close'] > spot_day.iloc[0]['Open'] else spot_day.iloc[0]['Open']
    CE_strike = round(CE_spot_price / 100) * 100

    PE_spot_price = spot_day.iloc[0]['Low']
    # PE_spot_price = spot_day.iloc[0]['Open'] if spot_day.iloc[0]['Open'] < spot_day.iloc[0]['Close'] else spot_day.iloc[0]['Close']
    PE_strike = round(PE_spot_price / 100) * 100


    time_delta2 = timedelta(hours=trade_time.hour, minutes=trade_time.minute, seconds=trade_time.second) # removed 1 hr to get spot time candle
    second_candle = current_day + time_delta2

    spot_day1 = spot_df[(spot_df['Date'] == second_candle)]
    vix_day = vix_df[(vix_df['Date'] == second_candle)]
    ...
    spot_price = spot_day1.iloc[0]['Open']
    spot_strike = round(spot_price / 100) * 100

    # CE_strike = spot_strike + 200
    # PE_strike = spot_strike - 200

    if (CE_spot_price - spot_price) > (spot_price - PE_spot_price):
        PE_strike = spot_price - (CE_spot_price - spot_price)
        PE_strike = round(PE_strike / 100) * 100
    
    if (CE_spot_price - spot_price) < (spot_price - PE_spot_price):
        CE_strike = spot_price + (spot_price - PE_spot_price)
        CE_strike = round(CE_strike / 100) * 100
    ...
    if os.path.exists(options_file_path):
        ...
        for i in range(len(call_sell_data)):
            ...
            if position == True:
                ...
                if call_sell_data['time'].iloc[i] > trade_time and call_sell_data['time'].iloc[i] < closing_time:
                    ...
                    if MTM_loss_hit == False:
                        ...
                        mtm_value = (call_sell_price - call_buy_price) * lot_size + (put_sell_price - put_buy_price) * lot_size
                ...
                if call_sell_data['time'].iloc[i] == closing_time:
                    ...
                    total_buy_price = ((call_buy_price + put_buy_price) * lot_size)

                    # if position == False:
                    if MTM_loss_hit:
                        final_trade_value = MTM_loss
                    else:
                        final_trade_value = (call_sell_price - call_buy_price) * lot_size + (put_sell_price - put_buy_price) * lot_size
            ...
            if position == False and call_sell_data['time'].iloc[i] == trade_time:
                ...
                total_sell_price = ((call_sell_price + put_sell_price) * lot_size)
...
df.to_csv('BNF_trading_data_EQ_2023_2024.csv', index=False)
...
# Try Get into SL position after market reverse

# =========================================================

# BankNifty Results:

# vix = 17 / SL 40 / MTM = -3500 / slippage = 1.005 / 
# lot_size = 30 / start_time = 10:15 / end_time = 14:45

# Initial amount              : 300000
# Total profit                : 84044.06
# Total brokerage             : 13608.82
# Net profit (after brokerage): 70435.24
# Net profit in percent       : 23.48%
# Max profit                  : 7884.09
# Max loss                    : -3661.00
# Profit count                : 91
# Loss count                  : 48
# Profit to loss ratio        : 1.90
# Max drawdown                : 4894.04
# Consecutive wins            : 7
# Consecutive losses          : 5
# Win percentage              : 65.47%
# Loss percentage             : 34.53%
