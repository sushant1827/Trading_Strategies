...
# df_list = [os.path.basename(file_path) for file_path in csv_files]
# print(df_list)
...
def get_value(signal, date, time, spot_price, init=False):
    ...
    for file in csv_files:
        ...
        # print(file_name)
        # file_date_range = file_name.split('_')#[0]  # Assuming the date range is always at the same position
        # print(file_date_range[1])
        # print(file_date_range[2])

        # start_date = file_date_range[1]
        # end_date   = file_date_range[2]
        
        # Extract start and end dates from the date range
        # start_date, end_date = file_date_range.split('-')
        ...
        # expiry_datetime = dt.datetime.combine(dt.datetime.strptime(start_date, "%Y-%m-%d").date(), start_time)

        # start_datetime = dt.datetime.combine(dt.datetime.strptime(start_date, "%Y-%m-%d").date(), start_time)
        # end_datetime   = dt.datetime.combine(dt.datetime.strptime(end_date, "%Y-%m-%d").date(), end_time)
        ...
        # Load the file into the dataframe only if it falls within the desired date range
        ...
        if list_file_name == search_file_name:
            ...
            # df['symbol1'] = df['symbol'].str.split('\d', expand=True)[0]
            # df['expiry'] = pd.to_datetime(df['symbol'].str.extract('(\d{2}\w{3}\d{2})')[0], format='%d%b%y')
            # df['strike'] = df['symbol'].str.extract('(\d+)').astype(int)
            # df['type'] = df['symbol'].str.extract('([A-Z]+)$')

            df[['symbol1', 'expiry', 'strike', 'type']] = df['symbol'].str.extract(r'^([A-Z]+)(\d{2}\w{3}\d{2})(\d+)([A-Z]+)$')



            # df.to_csv('df_after_change.csv')

            # print(temp_df)
            # print(type(temp_df['date']))

            # search_result = temp_df[
            #     (temp_df.iloc[:, 0] == datetime) &
            #     (temp_df['strike_price'] == strike_price) &
            #     (temp_df['right'] == right)
            # ]

            # print('*************************')
            # print(date, time, strike, right)
            # print(type(date))
            # print(type(time))
            # print(type(strike))
            # print(type(right))
            ...
            # print(df['date'][0] == str(date))
            # print(df['time'][0] == str(time))
            # print(df['strike'][0] == strike)
            # print(df['type'][0] == right)
    ...
    return open_price, strike, right
...
def backtest(df):
    ...
    for i in range(len(df)):
        ...
        if signal == 'Sell' and Buy_ON == True:
            ...
            trade_df.loc[j] = (['Closed Buy', str(df.index[i]), str(strike_price), option_type, str(buy_sell_value), str(profit_loss), str(total_profit_loss)])
        ...
        if signal == 'Buy' and Sell_ON == True:
            ...
            trade_df.loc[j] = (['Closed Sell', str(df.index[i]), str(strike_price), option_type, str(sell_buy_value), str(profit_loss), str(total_profit_loss)])
        ...
        if signal == 'Buy' and Buy_ON == False:
            ...
            buy_premium, strike_price, option_type = get_value(signal, date, time, spot_price, init=True)
        ...
        if signal == 'Sell' and Sell_ON == False:
            ...
            sell_premium, strike_price, option_type = get_value(signal, date, time, spot_price, init=True)
