# supertrend_heikin_ashi_with_filters.py
import pandas as pd
import numpy as np
import glob
import os
...
ADX_PERIOD = 14
ADX_THRESHOLD = 25
ATR_LOW_MULTIPLIER = 0.5  # ATR < ATR_LOW_MULTIPLIER * mean_ATR skip
...
def calculate_ema(df, period):
    if TALIB_AVAILABLE:
        ema_values = ta.EMA(df['close'].values, timeperiod=period)
...
def calculate_adx(df, period):
    if TALIB_AVAILABLE:
        adx_values = ta.ADX(df['high'].values, df['low'].values, df['close'].values, timeperiod=period)
        return pd.Series(adx_values, index=df.index)
    else:
        # Dummy implementation, replace with proper ADX calculation if needed
        return pd.Series([25.0] * len(df), index=df.index)

def calculate_rsi(df, period):
    if TALIB_AVAILABLE:
        rsi_values = ta.RSI(df['close'].values, timeperiod=period)
...
def calculate_bb(df):
    if TALIB_AVAILABLE:
        upper, middle, lower = ta.BBANDS(df['close'].values, timeperiod=BB_PERIOD, nbdevup=BB_STD, nbdevdn=BB_STD, matype=0)
...
def apply_filters(df, buy_signal, sell_signal, atr, ema_fast, ema_slow, adx, rsi, macd, macdsignal, upper_bb, lower_bb):
    # Trend confirmation
    trend_condition_buy = ((ema_fast > ema_slow) & (adx > ADX_THRESHOLD)).fillna(False)
    trend_condition_sell = ((ema_fast < ema_slow) & (adx > ADX_THRESHOLD)).fillna(False)

    # Volatility filter
    mean_atr = atr.mean()
    atr_low = ATR_LOW_MULTIPLIER * mean_atr
    atr_high = ATR_HIGH_MULTIPLIER * mean_atr
    vol_condition = ((atr > atr_low) & (atr < atr_high)).fillna(False)

    # RSI filter
    rsi_buy = (rsi < RSI_BUY_THRESHOLD).fillna(False)
    rsi_sell = (rsi > RSI_SELL_THRESHOLD).fillna(False)

    # MACD filter
    macd_buy = (macd > macdsignal).fillna(False)
    macd_sell = (macd < macdsignal).fillna(False)

    # Bollinger Bands filter
    bb_buy = (df['close'] < lower_bb).fillna(False)
    bb_sell = (df['close'] > upper_bb).fillna(False)

    # Time filter
    time_condition = (df.index.hour >= START_HOUR) & (df.index.hour <= END_HOUR)

    # Apply filters to buy signals
    buy_filtered = buy_signal.copy()
    if USE_TREND_FILTER:
        buy_filtered &= trend_condition_buy
    if USE_VOLATILITY_FILTER:
        buy_filtered &= vol_condition
    if USE_RSI_FILTER:
        buy_filtered &= rsi_buy
    if USE_MACD_FILTER:
        buy_filtered &= macd_buy
    if USE_BB_FILTER:
        buy_filtered &= bb_buy
    if USE_TIME_FILTER:
        buy_filtered &= time_condition

    # Apply filters to sell signals
    sell_filtered = sell_signal.copy()
    if USE_TREND_FILTER:
        sell_filtered &= trend_condition_sell
    if USE_VOLATILITY_FILTER:
        sell_filtered &= vol_condition
    if USE_RSI_FILTER:
        sell_filtered &= rsi_sell
    if USE_MACD_FILTER:
        sell_filtered &= macd_sell
    if USE_BB_FILTER:
        sell_filtered &= bb_sell
    if USE_TIME_FILTER:
        sell_filtered &= time_condition

    # Fill NaN as False
    buy_filtered = buy_filtered.fillna(False)
    sell_filtered = sell_filtered.fillna(False)
...
if __name__ == "__main__":
    ...
    upper_bb, middle_bb, lower_bb = calculate_bb(df)

    buy_signal, sell_signal = generate_signals_from_trend(trend)

    # Apply filters
    buy_filtered, sell_filtered = apply_filters(df, buy_signal, sell_signal, atr, ema_fast, ema_slow, adx, rsi, macd, macdsignal, upper_bb, lower_bb)
    ...
    combined['buy_filtered'] = buy_filtered.astype(int)
    combined['sell_filtered'] = sell_filtered.astype(int)
    combined.to_csv('supertrend_heikin_ashi_with_filters_output.csv')
    print("Saved calculations to supertrend_heikin_ashi_with_filters_output.csv")

    # backtest with filtered signals
    trades_df, final_capital = backtest_signals(df, buy_filtered, sell_filtered, atr)
