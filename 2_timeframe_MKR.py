...
# Based on the `2_timeframe_MKR.py` code, here are the detailed entry and exit conditions for buy and sell signals:

# ## Entry Conditions

# __Buy Signal (Go Long):__

# - Current position = 0 (no position)
# - 15-minute signal = 1 (positive/bullish)
# - 60-minute signal = 1 (positive/bullish)
# - Both timeframes must be aligned positively
# - Entry executed at current candle's open price

# __Sell Signal (Go Short):__

# - Current position = 0 (no position)
# - 15-minute signal = -1 (negative/bearish)
# - 60-minute signal = -1 (negative/bearish)
# - Both timeframes must be aligned negatively
# - Entry executed at current candle's open price

# ## Exit Conditions

# __Exit Long Position:__

# - Currently holding a long position (position = 1)
# - Either 15-minute signal = -1 OR 60-minute signal = -1
# - Exit executed at current candle's open price
# - Profit/Loss calculated as: exit_price - entry_price

# __Exit Short Position:__

# - Currently holding a short position (position = -1)
# - Either 15-minute signal = 1 OR 60-minute signal = 1
# - Exit executed at current candle's open price
# - Profit/Loss calculated as: entry_price - exit_price

# ## Key Implementation Details

# 1. __Look-Ahead Bias Prevention__: Uses signals from the previous candle (`signal_15min = df_1min['signal_15min'].iloc[i-1]`)

# 2. __Signal Alignment__: Both timeframes must confirm the same direction for entry, but only one timeframe needs to reverse for exit

# 3. __Position Management__:

#    - position = 0: No position
#    - position = 1: Long position
#    - position = -1: Short position

# 4. __Execution__: All entries and exits occur at the open price of the current candle to maintain realistic trading conditions

# 5. __Signal Generation__: MKR signals are generated separately for each timeframe and then merged onto the 1-minute dataframe with forward-filling to ensure continuity

# The strategy follows a "both confirm for entry, either confirms for exit" approach, which means it's more conservative on entering trades but quicker to exit when either timeframe shows a reversal.
...
def run_backtest(df_1min: pd.DataFrame, kernel: str, bandwidths: dict, timeframes: dict, initial_capital: float = 100000, return_signals: bool = False):
    """
    Runs a backtest for the two-timeframe MKR strategy.

    Parameters:
    df_1min (pd.DataFrame): 1-minute OHLC data.
    kernel (str): Kernel type for MKR.
    bandwidths (dict): Dictionary of bandwidths for each timeframe (e.g., {'15T': 20, '60T': 30}).
    timeframes (dict): Dictionary of timeframes to use (e.g., {'tf1': '15T', 'tf2': '60T'}).
    ...
    """
    df_15min = resample_ohlc(df_1min, timeframes['tf1'])
    df_60min = resample_ohlc(df_1min, timeframes['tf2'])

    # Apply MKR to each timeframe
    mkr_15min = MultiKernelRegression(bandwidth=bandwidths[timeframes['tf1']], kernel=kernel)
    df_15min = mkr_15min.generate_signals(df_15min)

    mkr_60min = MultiKernelRegression(bandwidth=bandwidths[timeframes['tf2']], kernel=kernel)
    ...
    # Strategy Logic - Two Timeframe Approach
    ...
    for i in range(1, len(df_1min)):
        ...
        # Entry Conditions - Both timeframes need to align
        if position == 0:
            # Buy signal: both timeframes positive
            if signal_15min == 1 and signal_60min == 1:
                position = 1
                entry_price = current_open  # Enter at open to avoid look-ahead bias
                entry_time = current_time
            # Sell signal: both timeframes negative
            elif signal_15min == -1 and signal_60min == -1:
                position = -1
                entry_price = current_open  # Enter at open to avoid look-ahead bias
                entry_time = current_time
        # Exit Conditions
        elif position == 1: # Currently Long
            # Exit if either timeframe turns negative
            if signal_15min == -1 or signal_60min == -1:
            ...
            # Exit if either timeframe turns positive
            if signal_15min == 1 or signal_60min == 1:
                exit_price = current_open  # Exit at open to avoid look-ahead bias
...
def optimize_strategy(df_1min: pd.DataFrame, initial_capital: float = 100000):
    """
    Optimizes the two-timeframe MKR strategy by iterating through parameters.
    """
    best_metrics = None
    best_params = {}
    ...
    kernels = ["Epanechnikov"] # Add more as needed "Logistic", "Triangular"
    
    # Bandwidth ranges for each timeframe
    # These are just examples, adjust based on typical data characteristics
    bandwidth_ranges = {
        '15T': range(23, 24, 1), # e.g., 25, 30, 35
        '60T': range(30, 31, 1)  # e.g., 35, 40, 45
    }

    # Timeframe combinations (2 timeframes only)
    timeframe_options = {
        'tf1': '15T',
        'tf2': '60T'
    }
    ...
    for kernel in kernels:
        for bw_15min in bandwidth_ranges['15T']:
            for bw_60min in bandwidth_ranges['60T']:
                ...
                print(f"Testing combination {current_combination}/{total_combinations}: Kernel={kernel}, BW_15min={bw_15min}, BW_60min={bw_60min}")
    ...
    if best_metrics:
        ...
        # Save signals to CSV
        signals_filename = f"mkr_signals_{best_params['kernel']}_15min{best_params['bandwidths']['15T']}_60min{best_params['bandwidths']['60T']}.csv"
        best_signals_df.to_csv(signals_filename)
        print(f"Signals saved to: {signals_filename}")
        
        # Save trades to CSV
        if not best_trades_df.empty:
            trades_filename = f"mkr_trades_{best_params['kernel']}_15min{best_params['bandwidths']['15T']}_60min{best_params['bandwidths']['60T']}.csv"
