# Enhanced Two Supertrend Gaussian Strategy with Profit Booking
# Strategy Total Profit (Points): Enhanced version with profit booking mechanism

# ----------------------------------------------------------------------------------------------

# Entry and exit conditions for buy and sell signals in the Enhanced Strategy:

# Buy Entry Conditions:
# - A potential buy signal is generated when both Supertrend indicators show upward trend (direction = 1)
...
# - A potential sell signal is generated when both Supertrend indicators show downward trend (direction = -1)
...
try:
    ...
    # Define trading parameters for position sizing
    initial_capital = 100000  # Starting capital
    risk_per_trade_pct = 0.02  # Risk per trade as % of capital (2%)
    target_pct = 0.0025  # Target percentage (0.2%)
    sl_pct = 0.0015  # Stop Loss percentage (0.2%)

    # Initialize MultiKernelRegression
    kernel = "Epanechnikov"
    mkr = MultiKernelRegression(bandwidth=9, kernel=kernel, deviations=2.0)
    ...
    supertrend_params = [(26, 0.9), (26, 0.9)]
    st_direction_cols = []
    ...
    for i, (period, multiplier) in enumerate(supertrend_params):
        ...
        st_result_ha = calculate_supertrend(df.copy(), period=period, multiplier=multiplier, suffix=suffix)
        st_col_name = f"ST_{period}_{str(multiplier).replace('.', '_')}{suffix}"
        st_dir_col_name = f"ST_Dir_{period}_{str(multiplier).replace('.', '_')}{suffix}"
        trending_up_col_name = f"trendingUp_HA_{period}_{str(multiplier).replace('.', '_')}{suffix}"
        trending_down_col_name = f"trendingDown_HA_{period}_{str(multiplier).replace('.', '_')}{suffix}"
    ...
    for i in range(1, len(df)):
        ...
        if supertrends_buy and mkr_buy and position_data['status'] != 'buy_position':
            df.loc[df.index[i], 'Buy_Signal'] = current_close

        # Generate sell signal (can be on same candle as buy signal)
        if supertrends_sell and mkr_sell and position_data['status'] != 'sell_position':
        ...
        if (df['Buy_Signal'].iloc[i-1] and not pd.isna(df['Buy_Signal'].iloc[i-1]) and
            ...
            position_data['target_1'] = entry_price + (target_profit * 1.0)  # 1.5x target
            position_data['target_2'] = entry_price + (target_profit * 2.0)  # 2.5x target
        ...
        # Manage existing positions
            ...
            # Check for profit booking at target 1 (40% position)
                ...
                profit_book_quantity = int(position_data['quantity'] * 0.4)
    ...
    if not trades_df.empty:
        ...
        print("\n--- Enhanced Trading Strategy Metrics ---")
        ...
        buy_hold_start_price = df.loc[start_date, 'Close']
        buy_hold_end_price = df.loc[end_date, 'Close']
        buy_hold_profit = buy_hold_end_price - buy_hold_start_price

        strategy_vs_buy_hold_diff = total_profit - buy_hold_profit
        strategy_vs_buy_hold_pct = (strategy_vs_buy_hold_diff / buy_hold_profit * 100) if buy_hold_profit != 0 else np.nan

        print("\n--- Enhanced Strategy vs Buy-and-Hold Comparison ---")
        print(f"Enhanced Strategy Total Profit (Points): {total_profit:.2f}")
        print(f"Buy-and-Hold Profit (Points): {buy_hold_profit:.2f}")
        print(f"Enhanced Strategy vs Buy-and-Hold: {'Better' if total_profit > buy_hold_profit else 'Worse' if total_profit < buy_hold_profit else 'Equal'} by {abs(total_profit - buy_hold_profit):.2f} points ({strategy_vs_buy_hold_pct:.2f}%)")

        # Count profit booking events
        profit_booking_1_count = len(trades_df[trades_df['exit_type'] == 'profit_booking_1'])
        profit_booking_2_count = len(trades_df[trades_df['exit_type'] == 'profit_booking_2'])
        full_exit_count = len(trades_df[trades_df['exit_type'] == 'full_exit'])

        print(f"\nProfit Booking Events:")
        print(f"Profit Booking at 1.5x Target (40% position): {profit_booking_1_count}")
        print(f"Profit Booking at 2.5x Target (40% position): {profit_booking_2_count}")
        print(f"Full Position Exits: {full_exit_count}")

    else:
        print("\nNo trades found to calculate metrics.")

    # Save the DataFrame to a CSV file
    output_csv_path = 'two_supertrend_gaussian_enhanced_signals.csv'
